...
router.post("/signup", async (req, res) => {
  try {
    const { name, emailId, password, phoneNumber, dept } = req.body;
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!media)
      return res.status(404).json({ message: "Communication form not found" });
    media.approvals.push({ department, user, date: new Date() });
    await media.save();
    res.json({
      message: "Communication form approved successfully",
      approvals: media.approvals,
    });
  }
);
